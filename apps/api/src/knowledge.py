...
from .knowledge_review import review_knowledge_version
...
def rrf_merge(
    ...
    for item in merged.values():
        item["rrf_score"] = (lexical_weight / (k + item.get("lexical_rank", 10_000))) + (
            1 / (k + item.get("vector_rank", 10_000))
        )
    return sorted(merged.values(), key=lambda row: (-row["rrf_score"], str(row["id"])))[:limit]


def filter_min_score(rows: list[dict[str, Any]], min_score: float) -> list[dict[str, Any]]:
    """Aplica limiar à similaridade vetorial normalizada; zero preserva o ranking."""
    if min_score == 0:
        return rows
    return [
        row
        for row in rows
        if float(row.get("vector_score") or 0) >= min_score
    ]
...
def create_source(
    ...
    def operation(connection: psycopg.Connection) -> tuple[int, dict[str, Any]]:
        row = connection.execute(
            "INSERT INTO mentor_concursos.knowledge_sources(owner_user_id,source_type,title,issuer,authority,canonical_url,license_status,trust_level,subject_id,metadata) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *",
            (
                user.id,
                body.source_type,
                body.title,
...
def list_sources(
    ...
    with _connect(settings) as connection:
        rows = connection.execute(
            "SELECT * FROM mentor_concursos.knowledge_sources WHERE owner_user_id=%s ORDER BY created_at DESC,id LIMIT %s",
            (user.id, limit),
        ).fetchall()
...
def get_source(
    source_id: uuid.UUID, user: UserDep, settings: Annotated[Settings, Depends(get_settings)]
) -> dict[str, Any]:
    with _connect(settings) as connection:
        row = connection.execute(
            "SELECT * FROM mentor_concursos.knowledge_sources WHERE id=%s AND owner_user_id=%s",
            (source_id, user.id),
        ).fetchone()
    if not row:
...
def create_version(
    ...
    def operation(connection: psycopg.Connection) -> tuple[int, dict[str, Any]]:
        if not connection.execute(
            "SELECT 1 FROM mentor_concursos.knowledge_sources WHERE id=%s AND owner_user_id=%s",
            (source_id, user.id),
        ).fetchone():
            raise HTTPException(status_code=404, detail="Fonte não encontrada")
        row = connection.execute(
            "INSERT INTO mentor_concursos.knowledge_source_versions(source_id,version_label,published_at,effective_from,effective_until,sha256,mime_type,byte_size,page_count,storage_key,extraction_metadata) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *",
            (
                source_id,
                body.version_label,
                body.published_at,
                ...
                body.page_count,
                body.storage_key,
                json.dumps(body.extraction_metadata),
            ),
        ).fetchone()
        _audit(
            connection,
    ...
    return _mutate(
        ...
        user,
        settings,
        body.model_dump(mode="json") | {"source_id": str(source_id)},
        operation,
    )
...
def request_ingest(
    ...
    def operation(connection: psycopg.Connection) -> tuple[int, dict[str, Any]]:
        if not connection.execute(
            "SELECT 1 FROM mentor_concursos.knowledge_source_versions v JOIN mentor_concursos.knowledge_sources s ON s.id=v.source_id WHERE v.id=%s AND s.owner_user_id=%s",
            (version_id, user.id),
        ).fetchone():
            raise HTTPException(status_code=404, detail="Versão não encontrada")
        key = request.headers["Idempotency-Key"]
        row = connection.execute(
            "INSERT INTO mentor_concursos.ingestion_jobs(source_version_id,requested_by,idempotency_key) VALUES (%s,%s,%s) ON CONFLICT (source_version_id,idempotency_key) DO UPDATE SET updated_at=CURRENT_TIMESTAMP RETURNING *",
            (version_id, user.id, key),
        ).fetchone()
        _audit(
            connection,
            "user",
...
def get_job(
    job_id: uuid.UUID, user: UserDep, settings: Annotated[Settings, Depends(get_settings)]
) -> dict[str, Any]:
    with _connect(settings) as connection:
        row = connection.execute(
            "SELECT j.* FROM mentor_concursos.ingestion_jobs j WHERE j.id=%s AND j.requested_by=%s",
            (job_id, user.id),
        ).fetchone()
    if not row:
...
def review_version(
    ...
    def operation(connection: psycopg.Connection) -> tuple[int, dict[str, Any]]:
        review = review_knowledge_version(
            connection,
    ...
    return _mutate(
        ...
        user,
        settings,
        body.model_dump(mode="json") | {"version_id": str(version_id)},
        operation,
    )
...
def retrieve(
    ...
    with _connect(settings) as connection:
        ...
        if explicit_locator:
            article_order = (
                "CASE WHEN regexp_replace(c.legal_locator, '\\.$', '')=%s THEN 0 ELSE 1 END"
            )
        ...
        k = 20
        lexical_weight = 4.0
        returned = filter_min_score(
            rrf_merge(lexical, vector_rows, body.limit, k, lexical_weight), body.min_score
        )
        scores = [
            {
                "chunk_id": str(row["id"]),
        ...
        connection.execute(
            "INSERT INTO mentor_concursos.retrieval_audit(user_id,query_hash,filters,returned_chunk_ids,scores,model_id,model_revision,request_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
            (
                user.id,
                query_hash,
                json.dumps(
    ...
    return {
        "items": [_row_response(row) for row in returned],
        "model_id": model_id,
        "model_revision": revision,
        "fusion": {"method": "rrf", "k": k, "lexical_weight": lexical_weight},
        "next_cursor": None,
    }
