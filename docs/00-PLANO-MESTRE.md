...
0. **Núcleo acadêmico determinístico (Fase 2B).** Modelo relacional, taxonomia
   referencial, objetivos, editais, ciclos, sessões, auditoria e idempotência.
   Gate: migração checksum-validada, Roberto provisionado sem duplicidade,
   nenhuma entidade fictícia em produção, API autenticada e métricas de sessão
   recalculáveis no servidor.
1. **Fundação técnica.** Estrutura, isolamento, API de saúde, worker, autenticação interna, migração controlada e validação real do núcleo. A Fase 1.2 acrescenta proxy com allowlist, métricas, rate limiting Redis, logs rotacionados e locks com hashes. Gate: seis serviços operacionais verificados, egress permitido/negado ensaiado, migração e vector validados, testes verdes, Hermes desativado e nenhuma porta ou segredo publicado.
